...
**Build-time data pipeline** (`build.rs`): Downloads airport/runway CSVs from OurAirports, filters to hard-surface runways and ICAO-coded airports, and generates `airport_db.rs` compiled into the binary. Falls back to cached CSVs in `data/` if download fails. Also reads TOML aircraft profiles from `data/aircraft/*.toml` and generates `aircraft_db.rs` with all built-in aircraft presets compiled into the binary.
...
**Aircraft data** (`data/aircraft/*.toml`): TOML profiles define aircraft performance (speeds, altitudes, fuel). Schema documented in `data/aircraft/FORMAT.md`. At runtime, users can also load custom profiles via `--profile path/to/custom.toml` or import Little Navmap performance files via the `aircraft import --format lnmperf` subcommand.

**Key design choices**:
- Airport/aircraft data use `'static` references (compile-time embedded)
- Runtime profile loading via `--profile` for custom aircraft without rebuilding
- LNM performance file import (`aircraft import`) for interop with Little Navmap
